...
Furthermore, in this tutorial, I'm assuming that you already have the Arduino IDE and Node.js software installed. 
...
Briefly, Proteus Design Suite is a combination of schematic capture, simulation and design of PCB, that help you to make a complete electronic circuit. Aside this, Proteus has the capability of simulate popular micro-controllers, executing it’s current firmware.
...
Using simulation in the Proteus software, instead of real physics devices, we have some advantages and disadvantages. To list someones:
...
### Installing and configure the Proteus and com0com software
...
Nice! Now download com0com from [here](http://sourceforge.net/projects/com0com/) and install it. During the installation process, it will create two virtual serial ports and maybe install the drivers. Let's call these two serial ports 'COM1' and 'COM2' for convenience. After the installation is complete, let's configure it to work with our simulated Arduino board and johnny-five app.

As [@randallagordon](https://github.com/randallagordon) point [here](https://github.com/rwaldron/johnny-five/issues/314#issuecomment-38187208), johnny-five use Firmata protocol to communicate with Arduino and because of this we need setup the baud-rate of our serial ports to **57600**. To do this, open your command-line and run the follow line:
...
Now, let's choose the components that will be used. In the menu, click in the "Library" option and select the "Pick parts from libraries" (or you can use the hotkey "P" - or click in the "P" button at the left panel). This will open a screen to you choose the device for your project. In the "Keyword" input, type "arduino". Next, select the "ARDUINO UNO R3" in the results list (double click to select). Now, type "COMPIM" in the search box again and select the "COMPIM" device listed. The last thing to choose is our Led device. Type "LED-RED" and select it in the list (you can choose other color if you prefer). Close the Pick Device window. You can see the three devices listed in the Devices panel at left.
...
Now that our components are connected, let's configure it to work accordingly.

First, double click at the Serial component and put the baud-rate option to 57600, stop bit to '1', parity to 'none' and so on. The same configuration that we did before to our virtual serial ports. Remember to define the Physical Port to 'COM1'.

![serial_configuration](https://cloud.githubusercontent.com/assets/641525/3139998/5efb016e-e8fe-11e3-99fe-cd19dca14a4c.png)

Now, in the same way that we make when we are working in physical world, we need upload the firmware to the Arduino component. 
...
Now, compile the code. As you can see in the console log at bottom, the code is compiled and the hexadecimal file are generated. Go to the folder that have this file and copy it (something like "C:\Users\user\AppData\Local\Temp\build123.tmp\StandardFirmata.cpp.hex"). Paste it in a local that is easy to find (I suggest to you that put it in the same folder of the Proteus project that we created before). 

Now, double click at Arduino board. In the configuration window of Arduino board that was opened, in the Firmware option, select the hexadecimal file that we copied before. Adjust the others options to match like the image bellow, if necessary:
...
Inside the "NodeProteusExample" folder that we created before, in your command-line, run the follow command.

```sh 
npm install johnny-five
```

This will download and install the johnny-five node module inside the "node_modules" sub-folder. As you can see, the johnny-five is here.
...
In this very simple code, we are creating a board that will connect to our virtual serial port 'COM2'. When the board is ready we are catching this event and executing a function that will create a new Led that will be connected to the pin 13 of this board and after that we are saying to strobe this Led on-off in 100ms phases.

### Running the example

Now that all components are created and connected, the configurations defined and our example written, let's run it.
...
Next, open your command-line in the "NodeProteusExample" folder and run:

```sh
node app.js
```

As you can see, the terminal will show that the program is connected to the serial port 'COM2' and starts send commands to it. In Proteus simulation, you can see that the LED component starts blink, as expected!
 
![running](https://cloud.githubusercontent.com/assets/641525/3208936/548ed638-ee4e-11e3-9c1c-1365cd2a1fc7.gif)

> **Note**: The LED component is turning on and off every 100ms but due the simulation update rate this time can be not so precise.

### Using the components that already comes with Proteus

If you would like use the components that already come with Proteus software, an easy way to start with this is create a new project in Proteus and select the "Arduino Pro 328" in Development Board section. This will create the Arduino board component in your sketch and, as you will see, a new tab is opened and displays the default Arduino code that will be compiled to generate de hexadecimal code (firmware) and uploaded to the Arduino board. 
...
Now that your Arduino board is OK put the Serial port and the LED component in the sketch and follow the steps that we did before to run the Blink example.
